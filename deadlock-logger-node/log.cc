...
log::log( SST::ComponentId_t id, SST::Params& params ) : SST::Component(id) {
    ...
    csvout.output("Time,Node,Node Status,Idle Time,Resource Requests\n");
...
bool log::tick( SST::Cycle_t currentCycle ) { 
    ...
    for(int i = 0; i < num_ports; ++i) {
        ...
        csvout.output("%ld,Node_%d,%d,%d,%d\n", getCurrentSimTime(), i, stateArray[i], idleArray[i], requestArray[i]);
    }
...
void log::messageHandler( SST::Event *ev ) { 
    ...
    if (le != NULL) { 
        ...
        stateArray[le->log.node_id] = le->log.node_status;
