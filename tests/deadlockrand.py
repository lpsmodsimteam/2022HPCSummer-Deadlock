...
for x in range(NUM_NODES - 1):
    sst.Link(f"Link_{x}").connect( (nodes[f'node_{x}'], "nextPort", "1ps" ), (nodes[f'node_{x + 1}'], "prevPort", "1ps") )

sst.Link(f"Link_{NUM_NODES - 1}").connect( (nodes[f'node_{NUM_NODES - 1}'], "nextPort", "1ps" ), (nodes['node_0'], "prevPort", "1ps") )